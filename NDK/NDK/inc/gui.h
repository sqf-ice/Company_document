...
typedef struct _surface_t {
	...
	int vbuf_len;			/**< 显示缓冲区大小 */
	surface_mode_t mode;	/**< 显示模式 */
	surface_ops_t * ops;	/**< 绘制函数 */
};
...
typedef enum {
	...
	SOCKET_DATA_ECHO = 0x0E,			/*无操作，用于响应同步*/
...
int socket_unix_close (int fd);
int socket_unix_request(int s, socket_header_t header, void * data, int len);
int socket_unix_response(int s, void * data, int reqlen, int timeout);
